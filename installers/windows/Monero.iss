; Nevocoin Fluorine Fermi GUI Wallet Installer for Windows
; Copyright (c) 2017-2020, The Monero Project
; See LICENSE
#define GuiVersion GetFileVersion("bin\nevocoin-wallet-gui.exe")

[Setup]
AppName=Nevocoin GUI Wallet
...
DefaultDirName={commonpf}\Nevocoin GUI Wallet
DefaultGroupName=Nevocoin GUI Wallet
...
LicenseFile=LICENSE
AppPublisher=The Nevocoin Developer Community
AppPublisherURL=https://nevocoin.org
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the Nevocoin files can be copied to a place where they have at least
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Nevocoin.

Source: {#file AddBackslash(SourcePath) + "ReadMe.htm"}; DestDir: "{app}"; DestName: "ReadMe.htm"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy
Source: "LICENSE"; DestDir: "{app}"; Flags: ignoreversion

; Nevocoin GUI wallet exe and guide
Source: "bin\nevocoin-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\nevocoin-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; Nevocoin CLI wallet
Source: "bin\extras\nevocoin-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nevocoin-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; Nevocoin wallet RPC interface implementation
Source: "bin\extras\nevocoin-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Nevocoin daemon
Source: "bin\nevocoind.exe"; DestDir: "{app}"; Flags: ignoreversion

; Nevocoin daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "nevocoin-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Nevocoin blockchain utilities
Source: "bin\extras\nevocoin-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nevocoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nevocoin-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nevocoin-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nevocoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nevocoin-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nevocoin-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nevocoin-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nevocoin-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nevocoin-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nevocoin-gen-ssl-cert.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\nevocoin-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\nevocoin');
  s := 'The default folder to store the Nevocoin blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\nevocoin\nevocoin-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Nevocoin default location
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "nevocoin-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Nevocoin daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\nevocoind.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\nevocoin-daemon.bat'), s, false); 
  end;
end;
...
Name: "{group}\Utilities\Nevocoin Daemon"; Filename: "{app}\nevocoind.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\nevocoin-wallet-cli.exe"; WorkingDir: "{userdocs}\Nevocoin\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\Nevocoin\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\nevocoin-wallet-gui\nevocoin-wallet-gui.log"""
...
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM nevocoind.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\nevocoin-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\nevocoin"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\nevocoin\nevocoin-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\nevocoin\nevocoin-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "nevocoin:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "nevocoin://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "nevocoin"; ValueType: "string"; ValueData: "URL:Nevocoin Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "nevocoin"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "nevocoin\DefaultIcon"; ValueType: "string"; ValueData: "{app}\nevocoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "nevocoin\shell\open\command"; ValueType: "string"; ValueData: """{app}\nevocoin-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "nevocoinseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "nevocoinseed://a.b.c.d:port"
Root: HKCR; Subkey: "nevocoinseed"; ValueType: "string"; ValueData: "URL:Nevocoin Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "nevocoinseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "nevocoinseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\nevocoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "nevocoinseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\nevocoin-wallet-gui.exe"" ""%1"""
